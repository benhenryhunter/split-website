...
function validateSignupForm(payload) {
  ...
  if (!payload || typeof payload.bdfmo !== 'string' || payload.bdfmo.trim().length === 0) {
    isFormValid = false;
    errors.bdfmo = 'Please provide your BD/FMO.';
  }

  if (!payload || typeof payload.company !== 'string' || payload.company.trim().length === 0) {
    isFormValid = false;
...
function validateAdminSignupForm(payload) {
  const errors = {};
  let isFormValid = true;
  let message = '';
...
router.post('/signup/admin', (req, res, next) => {
  const validationResult = validateAdminSignupForm(req.body);
  ...
  return passport.authenticate('local-signup-admin', (err) => {
    if (err) {
